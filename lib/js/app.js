$(function(){
        ...
        for (let i=0; i<$choicesBtn.length; i++){
            $choicesBtn[i].addEventListener("click", function validateAssessment(){
                if (this.getAttribute("value") == 1){
                    ...
                    showModal();
                    this.parentNode.parentNode.parentNode.children[0].children[2].children[0].children[0].checked = true;
    ...
    let $afterVidBtn = document.querySelector("#afterVidBtn")
    ...
    $welcomeVid.addEventListener("timeupdate",function(){
        if($welcomeVid.currentTime >= 93){
            //console.log("OK na");
            $afterVidBtn.style.display = "block";
